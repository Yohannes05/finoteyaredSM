...
export default function NewDeaconPage() {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [imageFile, setImageFile] = useState<File | null>(null)
  const { t } = useLanguage()
  const [formData, setFormData] = useState({
    first_name: "", last_name: "", phone: "", photo_url: "", ordination_date: getTodayEthioDate()
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('deacons').insert([dataToInsert])
      if(!error) {
        toast.success(t('deacon_new_success'))
  ...
  return (
    ...
    >
      <div className="space-y-1 text-center sm:text-left">
        <h2 className="text-3xl font-bold tracking-tight text-slate-900">{t('deacon_new_title')}</h2>
        <p className="text-sm text-slate-500 mt-0.5">{t('deacon_new_subtitle')}</p>
      </div>
      ...
      <Card className="shadow-xl border-slate-200 overflow-hidden">
        <form onSubmit={handleSubmit}>
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 mb-6">
            ...
            <CardDescription>{t('deacon_new_subtitle')}</CardDescription>
          </CardHeader>
