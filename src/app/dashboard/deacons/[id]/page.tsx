...
import { Phone, Calendar, Cross, Activity, X, User, CheckCircle2, XCircle, BookOpen, Trash2, AlertTriangle } from "lucide-react"
...
export default function DeaconDetailsPage({ params }: { params: { id: string } }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (deacon?.photo_url) {
        const photoPath = deacon.photo_url.split('/').pop()
        if (photoPath) {
          await supabase.storage.from('avatars').remove([photoPath])
        }
      }

      // Deletes deacon (from students table); CASCADE handles deacon_schedules
      const { error } = await supabase.from('students').delete().eq('id', deaconId)
      if (error) throw error
      toast.success(t('delete_success_deacon'))
      router.push('/dashboard/deacons')
    } catch (err: any) {
      ...
      <div className="relative rounded-3xl overflow-hidden shadow-sm border border-slate-200/60 bg-white">
        <div className="pt-16 sm:pt-20 px-4 sm:px-10 pb-6 sm:pb-8 flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-6 relative z-10">
          ...
          <div className="shrink-0 mt-2 sm:mt-0 w-full sm:w-auto flex items-center gap-2">
            <button onClick={() => router.push(`/dashboard/deacons/${deaconId}/edit`)} className="px-6 py-2.5 rounded-xl text-sm font-bold bg-slate-900 text-white hover:bg-slate-800 shadow-lg shadow-slate-200 transition-all active:scale-95 flex items-center justify-center gap-2">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900">
                    {t('delete_confirm_title').replace('{{name}}', `${deacon.first_name} ${deacon.last_name}`)}
                  </h3>
