...
export default function StudentDetailsPage({ params }: { params: { id: string } }) {
  ...
  const handleToggleAccepted = async () => {
    if (!student) return
    const newValue = !student.deacon_accepted
    setIsUpdatingAccepted(true)
    try {
      const { error } = await supabase.from('students').update({ deacon_accepted: newValue }).eq('id', studentId)
      if (error) throw error
      setStudent({ ...student, deacon_accepted: newValue })
               ...
               {student.is_deacon && (
                 <div className="flex items-center gap-4">
                   <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 border shadow-sm ${
                     student.deacon_accepted
                       ? 'bg-emerald-50 border-emerald-100'
                       : 'bg-slate-100 border-slate-200'
                   }`}>
                     <CheckCircle2 className={`h-5 w-5 ${student.deacon_accepted ? 'text-emerald-500' : 'text-slate-300'}`} />
                   </div>
                   <div className="flex-1">
                     ...
                     <label className="flex items-center gap-3 cursor-pointer group" onClick={handleToggleAccepted}>
                       <div className="relative">
                         <input
                           type="checkbox"
                           checked={student.deacon_accepted || false}
                           onChange={() => {}}
                       ...
                       <span className={`text-sm font-bold transition-colors ${
                         ...
                         {student.deacon_accepted ? t('deacon_accepted_yes') : t('deacon_accepted_no')}
                       </span>
                       {isUpdatingAccepted && (
